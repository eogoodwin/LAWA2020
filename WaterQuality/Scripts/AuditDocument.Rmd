...
wqtrend=wqtrend%>%
  select(-ConfCat,-frequency,-nMeasures,-note,-nReq,-ConfCat)%>%
...
rm(tab2)
if(!is.na(unique(wqdata$Units[wqdata$Agency==agency]))){
...
toLog=data.frame(msms=c("PH", "TP", "TN", "NH4", "TON", "BDISC", "TURB", "ECOLI", "DRP","TURBFNU"),
                 logy=c("","y","y","y","y","y","y","y","y","y"),stringsAsFactors=F)
...
This year there are many more results shown here, as we're now looking at historical five-year periods as well.
...
if(any(nfts$NitrateAnalysisNote!="")){
  cat('  \n### Nitrate\n\n')
  print(knitr::kable(nfts%>%dplyr::filter(NitrateAnalysisNote!=''&Year=="2015to2019")%>%
                       dplyr::select(CouncilSiteID,Catchment,Year,NitrateMed:NitrateAnalysisNote)%>%
...
if(any(nfts$AmmoniaAnalysisNote!="")){
  cat('  \n### Ammonia\n\n')
  print(knitr::kable(nfts%>%filter(NitrateAnalysisNote!=''&Year=="2015to2019")%>%
                       dplyr::select(CouncilSiteID,Catchment,Year,NitrateMed:NitrateAnalysisNote)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'Nitrate|Nitrate_',replacement = '',ignore.case=T))))
  # rownames=F,extensions='Buttons',options=list(dom='Bf',buttons=c('copy','csv','excel')))
  cat('  \n')
}
```
```{r echo=F,results='asis'}
if(any(!is.na(nfts$E_coliAnalysisNote)&nfts$E_coliAnalysisNote!="")){
  ...
  print(knitr::kable(nfts%>%filter(E_coliAnalysisNote!=''&Year=="2015to2019")%>%
                       dplyr::select(CouncilSiteID,Catchment,Year,E_coli_Period,E_coliSummaryband,E_coliAnalysisNote)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F))))
  # rownames=F,extensions='Buttons',options=list(dom='Bfpl',pagelength=10,buttons=c('copy','csv','excel')))
  cat('  \n')
  print(knitr::kable(nfts%>%filter(E_coliAnalysisNote!=''&Year=="2015to2019")%>%
                       dplyr::select(CouncilSiteID,Catchment,Year,E_coli_Median:E_coliRecHealth260_Band)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F))))
  # rownames=F,extensions='Buttons',options=list(dom='Bfpl',pagelength=10,buttons=c('copy','csv','excel')))
  cat('  \n')
}
...
DT::datatable(wqtrend,extensions=c('Buttons','ColReorder','FixedColumns'),
              options=list(dom='Bfrtip',colReorder=T,buttons=c('colvis','copy','csv','excel'),scrollX=TRUE,fixedColumns=list(leftColumns=5)),
