...
agencyFiles = data.frame(name=tail(dir(path = "H:/ericg/16666LAWA/LAWA2020/WaterQuality/Data/",
...
wqAudit = read.csv(tail(dir(path="H:/ericg/16666LAWA/LAWA2020/WaterQuality/Audit",
                            pattern = "WQAudit.*csv",
                            recursive = T,full.names = T,ignore.case = T),1),
                   stringsAsFactors = F)

agencyCSV=loadLatestCSVRiver(agency = agency,maxHistory = 30)
...
agencyRep=read.csv('h:/ericg/16666LAWA/LAWA2020/Metadata/AgencyRepWFS.csv',check.names = F)

#RiverState Summary
ss <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/WaterQuality/Analysis",
...
nfts <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/WaterQuality/Analysis",
                        pattern = 'NOFSummaryTable_Rolling.csv',  #Was ~_Overall until 2 July2020
                      full.names=T,recursive=T,ignore.case=T),1),stringsAsFactors=F)
nfts <- nfts[which(nfts$LawaSiteID%in%agencyLIDs),]
...
wqtrend=read_csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/WaterQuality/Analysis",
                          pattern="RiverWQ_Trend*.*",
                          full.names=T,recursive=T,ignore.case=T),1),guess_max = 20000)
wqtrend <- wqtrend[which(wqtrend$LawaSiteID%in%agencyLIDs),]
wqtrend <- wqtrend%>%filter(Measurement!="MCI")
...
wqtrend <- wqtrend%>%select(CouncilSiteID,SiteID,LawaSiteID,Measurement,nMeasures,frequency,period,TrendScore,ConfCat)
...
We hope to report on BDISC, TURB, NH4, PH, TON, TN, DRP, TP, and ECOLI, and these go by various nomenclatures at different councils.  The list of measurement names that I call from `r agency` is `r paste(Measurements,collapse=', ')`. It would be great to know whether this list is correct, if some could be removed, or others should be added.
...
if((length(missingCSIDs))>0){
  ...
  cat(length(missingCSIDs),
      " of the CouncilSiteIDs announced on the WFS are missing from the timeseries server.\nThese are:\n",
paste(missingCSIDs,collapse=', '),'.\nThey may well simply be named differently between WFS and time series server, but that\'s enough to thoroughly confuse me!\n',sep='')
  DT::datatable(siteTable%>%
                  filter(CouncilSiteID%in%missingCSIDs|tolower(SiteID)%in%missingCSIDs)%>%
...
print(paste("Data was last harvested from the timeseries server",checkXMLageRiver(agency),"days before",as.character(Sys.Date())))
```
...
cat('  \n')
if(any(!c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")%in%unique(wqAudit$agency))){
cat(c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")[!c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")%in%unique(wqAudit$agency)]," currently no data available.")
}
```
...
if(any(agencyCSVn$n>1)){
  cat("Yep, you've got remplicates alright, in your data.\n\n")

  reps = agencyCSVn%>%filter(n>1)%>%distinct
...
```{r echo=F,results='asis'}
cat('  \n### BDisc\n')
knitr::include_graphics(tail(dir(path="H:/ericg/16666LAWA/LAWA2020/WaterQuality/Audit",
                                 pattern="BDISC.*png",
                                 recursive = T,full.names = T,ignore.case = T),1))
cat('  \n  \n### DRP\n')
knitr::include_graphics(tail(dir(path="H:/ericg/16666LAWA/LAWA2020/WaterQuality/Audit",
...
toLog=data.frame(msms=c("PH", "TP", "TN", "NH4", "TON", "BDISC", "TURB", "ECOLI", "DRP"),
                 logy=c("","y","y","y","y","y","y","y","y"),stringsAsFactors=F)
...
DT::datatable(ss%>%
                ...
                dplyr::mutate(Median=signif(Median,2))%>%spread(Parameter,Median),rownames = F)
```
...
```{r echo=F,results='asis'}
DT::datatable(ss%>%dplyr::select(CouncilSiteID,Catchment,Parameter,StateScore)%>%spread(Parameter,StateScore))
```
...
```{r echo=F,results='asis'}
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,Catchment,contains('band'))%>%
                     rename_all(.funs = ~gsub(x=.,pattern = '_band|band',replacement = '',ignore.case = T)),rownames = F)
cat('  \n')

if(any(nfts$NitrateAnalysisNote!="")){
  cat('  \n### Nitrate\n')
  DT::datatable(nfts%>%filter(NitrateAnalysisNote!='')%>%
                       dplyr::select(CouncilSiteID,Catchment,NitrateMed:NitrateAnalysisNote)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'Nitrate|Nitrate_',replacement = '',ignore.case=T)),rownames=F)
  cat('  \n')
}
...
DT::datatable(nfts%>%filter(E_coliAnalysisNote!='')%>%
                       dplyr::select(CouncilSiteID,Catchment,E_coli_Period,E_coliSummaryband,E_coliAnalysisNote)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F)),rownames=F)
    cat('  \n')
  DT::datatable(nfts%>%filter(E_coliAnalysisNote!='')%>%
                       dplyr::select(CouncilSiteID,Catchment,E_coli_Median:E_coliRecHealth260_Band)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F)),rownames=F)
  cat('  \n')
}
...
```{r echo=F,results='asis'}
cat(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/WaterQuality/Analysis",
                          pattern="RiverWQ_Trend*.*",
                          full.names=F,recursive=T,ignore.case=T),1),' \n\n')
DT::datatable(wqtrend,extensions=c('Buttons','ColReorder','FixedColumns'),
              options=list(dom='Bfrtip',colReorder=T,buttons=c('colvis','csv'),scrollX=TRUE,fixedColumns=list(leftColumns=5)),
