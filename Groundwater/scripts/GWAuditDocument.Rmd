...
gwAudit = read.csv(tail(dir(path="H:/ericg/16666LAWA/LAWA2019/GW/Audit",
...
ss <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2019/GW/Analysis",
                        pattern = 'ITEGWState',
						full.names = T,recursive=T,ignore.case=T),1),stringsAsFactors = F)
ss=ss[which(tolower(ss$LawaSiteID)%in%agencyLIDs),]

GWtrend=read_csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2019/GW/Analysis",
                          pattern="*GWTrend*.*",
...
if(dim(GWtrend)[1]>0){
GWtrend$nReq = NA
GWtrend$nReq[GWtrend$frequency=='quarterly']=GWtrend$period[GWtrend$frequency=='quarterly']*4*0.75
GWtrend$nReq[GWtrend$frequency=='monthly']=GWtrend$period[GWtrend$frequency=='monthly']*12*0.75
...
print(knitr::kable(gwAudit[which(gwAudit$Agency==agency),],format.args=list(scientific=5))%>%
        kableExtra::kable_styling(c('bordered','striped'),full_width=T))
```
...
```{r echo=F,results='asis'}
cat('  \n### Chloride\n')
knitr::include_graphics(tail(dir(path="H:/ericg/16666LAWA/LAWA2019/GW/Audit",
...
for(msm in unique(agencyGWdata$Measurement)){
  ...
  for(lsi in sort(unique(agencyGWdata$LawaSiteID))){
    tsDat=bpDat%>%filter(LawaSiteID==lsi)
    if(dim(tsDat)[1]>0){
...
print(knitr::kable(spread(data=ss%>%
                            ...
                            dplyr::mutate(median=signif(median,2)),Measurement,median))%>%
        kableExtra::kable_styling(c('bordered','striped'),full_width=T))
```
...
```{r echo=F,results='asis'}
cat(tail(dir(path = "h:/ericg/16666LAWA/LAWA2019/GW/Analysis",
                          pattern="ITEGWTrend*.*",
                          full.names=T,recursive=T,ignore.case=T),1),' \n\n')
if(!is.null(GWtrend)){
print(knitr::kable(GWtrend)%>%
        kableExtra::kable_styling(c('bordered','striped'),full_width=T))
}else{
