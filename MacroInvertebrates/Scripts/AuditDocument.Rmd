...
macroTrend <- macroTrend%>%select(CouncilSiteID,SiteID,Measurement,nMeasures,frequency,period,TrendScore,ConfCat)
...
macroTrend=macroTrend%>%distinct%>%select(-frequency,-nMeasures,-note,-nReq)
# %>%tidyr::spread(key = c('Measurement','period'),value="TrendScore")
...
```{r echo=F, results='asis'}
DT::datatable(macroAudit%>%dplyr::group_by(agency)%>%dplyr::summarise(totalSites=max(nSite,na.rm=T),
...
Trends are in 5 categories, with -2 meaning a likely degrading trend, and 2 meaning a likely improving trend. 0 is indeterminate, and NA when there is insufficient data to calculate a trend. For 10 year trends we need 9 years to be represented, and for 15 year trends we need 13 years with data.
...
```{r echo=F,results='asis'}
DT::datatable(macroTrend,extensions=c('Buttons','ColReorder','FixedColumns'),
