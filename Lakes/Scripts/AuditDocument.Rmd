...
agencyCSVn = agencyCSV%>%group_by(Date,CouncilSiteID,Measurement)%>%
  transmute(n=n(),min=min(Value),max=max(Value),range=max-min,mean=signif(mean(Value),2),median=median(Value))%>%ungroup
...
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,contains('band'))%>%
                ...
                extensions='Buttons',options=list(dom='Bf',buttons=c('copy','csv','excel')))
cat('  \n')
