...
agencyFiles = data.frame(name=dir(path = "H:/ericg/16666LAWA/LAWA2020/Lakes/Data/",
...
LWQaudit = read.csv(tail(dir(path="H:/ericg/16666LAWA/LAWA2020/Lakes/Audit",
...
agencyRep=read.csv('h:/ericg/16666LAWA/LAWA2020/Metadata/AgencyRepLakeWFS.csv',check.names = F)

ss <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/Lakes/Analysis",
...
nfts <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/Lakes/Analysis",
...
TLI <- read.csv(tail(dir(path = "h:/ericg/16666LAWA/LAWA2020/Lakes/Analysis",
                         pattern='auditLakeTLI.*csv',
...
if(dim(lwqtrend)[1]>0){
  ...
  lwqtrend=lwqtrend%>%
    select(-LawaSiteID,-frequency,-nMeasures,-note,-nReq)%>%
...
if(length(missingCSIDs)>0){
  ...
  cat(length(missingCSIDs),
      " of the sites announced on the WFS are missing from the timeseries server.\nThese are:\n",
...
cat('  \n')
if(any(!c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")%in%unique(LWQaudit$agency))){
  cat(c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")[!c("arc","boprc","ecan","es","gdc","gwrc","hbrc","hrc","mdc","ncc","nrc","orc","tdc","trc","wcrc","wrc")%in%unique(LWQaudit$agency)]," currently no data available.")
}
```
...
```{r echo=F,results='asis'}
cat('  \n### TN  \n  ')
knitr::include_graphics(tail(dir(path="H:/ericg/16666LAWA/LAWA2020/Lakes/Audit",
...
for(msm in unique(agencyLWQdata$Measurement)){
  ...
  for(csi in sort(unique(agencyLWQdata$CouncilSiteID))){
...
These require 30 data points in five years before they'll be calculated, otherwise reported as NA.

```{r echo=F,results='asis'}
if(dim(ss)[1]>0){
  DT::datatable(spread(data=ss%>%
                              dplyr::select(CouncilSiteID,SiteID,LAWAID,Catchment,Parameter,Median)%>%
...
DT::datatable(TLI%>%drop_na(Value)%>%
                transmute(CouncilSiteID=factor(SiteID),
                          ...
                          LFENZID=factor(LFENZID),Year,TLI=signif(Value,3),Med5=signif(median5yr,3)),
              extensions='RowGroup',options=list(rowGroup=list(dataSrc=2),pageLength=12),filter='top',rownames = F)
```
...
```{r echo=F,results='asis'}
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,contains('band'))%>%
                     rename_all(.funs = ~gsub(x=.,pattern = '_band|band',replacement = '',ignore.case = T)))
cat('  \n')

if(any(nfts$NitrateAnalysisNote!="")){
  cat('  \n### Nitrogen\n')
  DT::datatable(nfts%>%dplyr::select(CouncilSiteID,Total_Nitrogen:NitrateAnalysisNote)%>%
...
if(any(nfts$PhosphorusAnalysisNote!="")){
  cat('  \n### Phosphorus\n')
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,Total_Phosphorus:PhosphorusAnalysisNote)%>%
...
if(any(nfts$AmmoniaAnalysisNote!="")){
  cat('  \n### Ammonia\n')
  DT::datatable(nfts%>%dplyr::select(CouncilSiteID,AmmoniacalMed:AmmoniaAnalysisNote)%>%
...
if(any(nfts$ClarityAnalysisNote!="")){
  cat('  \n### Clarity\n')
  DT::datatable(nfts%>%dplyr::select(CouncilSiteID,Median_Clarity:ClarityAnalysisNote)%>%
...
if(any(nfts$ChlAAnalysisNote!="")){
  cat('  \n### ChlA\n')
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,Median_ChlA:ChlAAnalysisNote)%>%
...
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,E_coli_Period,E_coliSummaryband,E_coliAnalysisNote)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F)))
    cat('  \n')
DT::datatable(nfts%>%dplyr::select(CouncilSiteID,E_coli_Median:E_coliRecHealth260_Band)%>%
                       rename_all(.funs = ~gsub(x=.,pattern = 'E_coli_|E_coli',replacement = '',ignore.case=F)))
    cat('  \n')
}
```
...
```{r echo=F,results='asis'}
DT::datatable(lwqtrend,extensions=c('Buttons','ColReorder','FixedColumns'),
